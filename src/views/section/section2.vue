...
const downloadSingleFile = async (row:any) => {
  ...
  await downloadFileUtil('section2',row.showName,row.fileName)
...
const getLists = async () => {
  try {
    const result:any = await getFileList('section2',currentPage.value,pageSize.value)
...
const handleRemoveFile =  async (row:any) => {
  const result:any =await deleteFile('section2',row.showName)
  if(result.code === 200){
    ElMessage({
      message: '删除成功',
      type: 'success'
...
const handleChangeUpload = async(file:File) =>{
  ...
  try {
    const result:any = await uploadFile({ bucket: 'section2', uploadfile: file.raw })
    if(result.code === 200){
      ElMessage({
        message: '上传成功',
        type: 'success'
