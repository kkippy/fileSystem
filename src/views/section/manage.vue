...
const downloadSingleFile = async (row:any) => {
  ...
  await downloadFileUtil('manage',row.showName,row.fileName)
...
const handleRemoveFile =  async (row:any) => {
  console.log(row)
  const result:any =await deleteFile('manage',row.showName)
  if(result.code === 200){
    ElMessage({
      message: '删除成功',
      type: 'success'
...
const handleChangeUpload = async(file:File) =>{
  ...
  try {
    const result:any = await uploadFile({ bucket: 'manage', uploadfile: file.raw })
    if(result.code === 200){
      ElMessage({
        message: '上传成功',
        type: 'success'
